...
func (repo *LinkTagsRepository) GetTagsForLink(linkId uuid.UUID, userInfo *Models.UserInfo) (*[]Models.Tag, error) {
	tagRows, err := repo.connection.QueryRows("select t.id, t.name, t.user_id from link_tags lt join tags t on lt.tag_id = t.id where lt.link_id = $1 and t.user_id = $2 and lt.user_id = $2", linkId, userInfo.UserId)
	if err != nil {
		log.Warningf("Failed to query for link_tags: %s", err.Error())
		return nil, err
	}
